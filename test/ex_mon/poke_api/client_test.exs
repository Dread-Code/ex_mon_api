defmodule ExMon.PokeApi.ClienTest do
  use ExUnit.Case
  import Tesla.Mock
  ...
  describe "get_pokemon/1" do
    ...
    test "when there is not a pokemon with the given name, returns an error" do
      mock(fn %{method: :get, url: "#{@base_url}bananon"} ->
        %Tesla.Env{
          status: 404
        }
      end)
      response = Client.get_pokemon("bananon")
      expected_response = {:error, "Pokemon not found"}
      assert response == expected_response
    end

    test "when there is an unexpected error, return an error" do
      mock(fn %{method: :get, url: "#{@base_url}pikachu"} ->
        {:error, :timeout}
      end)
      response = Client.get_pokemon("pikachu")
